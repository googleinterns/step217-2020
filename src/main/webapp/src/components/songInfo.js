...
class SongInfo extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (!objectsEqual(prevState.entityState, this.state)) {
      const json = JSON.stringify(this.state);
      localStorage.setItem("state", json);
    }
  }
    ...
    return (
      <div className={classes.root}>
        ...
        <div className={classes.youTubeRecommendationsSection}>
          ...
          <YouTubeRecommendations entityState={this.state.entityState} maxResults={5} onChangeState={this.handleYouTubeChange}/>
        </div>
      </div>
    );
  }
